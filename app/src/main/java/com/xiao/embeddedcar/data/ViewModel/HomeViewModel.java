...
import static com.xiao.embeddedcar.ui.activity.MainActivity.chief_status_flag;
            ...
            if (mByte[0] == (byte) 0xAA) {
                ...
                if (mByte[1] == (byte) 0x12) {
                    ...
                    if (mByte[2] == (byte) 0x0A) {
                        ...
                        MainApplication.cachedThreadPool.execute(() -> ConnectTransport.getInstance().RFID1(mByte[3]));
                    }
                    /* 检测到卡2处理 */
                    if (mByte[2] == (byte) 0x0B) {
                        debugArea.setValue("卡(2)数据:\n" + Arrays.toString(data));
                        ConnectTransport.setRFID2(data);
                        MainApplication.cachedThreadPool.execute(() -> ConnectTransport.getInstance().RFID2(mByte[3]));
                    }
                ...
                if (FastDo.isFastSend()) {
                    ...
                    MainApplication.cachedThreadPool.execute(() -> ConnectTransport.getInstance().half_Android());
                }
                commandData.setValue(mByte);
            }
            return true;
