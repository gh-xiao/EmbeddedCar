...
public class MainViewModel extends ViewModel {
    ...
    private void connect_thread() {
        MainApplication.cachedThreadPool.execute(() -> ConnectTransport.getInstance().connect( MainActivity.getLoginInfo().getIP()));
    }
    ...
    private void serial_thread() {
        MainApplication.cachedThreadPool.execute(() -> ConnectTransport.getInstance().serial_connect());
    }
    ...
    public void getCameraPicture() {
        if (ready) {
            ...
            ready = !ready;
            //单线程池
            MainApplication.singleThreadExecutor.execute(() -> ConnectTransport.getInstance().getPicture(getBitmapHandle));
        }
    }
